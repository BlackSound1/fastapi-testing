...
async def test_get_posts_empty(client: AsyncClient):
    """
    Tests that there are no posts at first.

    :param client: The DB client to use. Found as a fixture from closest conftest file.
    """
...
async def test_get_post_not_found(client: AsyncClient):
    """
    Tests that a non-existent post doesn't exist.
...
async def test_create_post_success(client: AsyncClient):
    """
    Tests that creating a post normally works.
...
@pytest.mark.anyio
async def test_create_post_not_authenticated(client: AsyncClient):
    """
    Tests that it's not possible to post when not authenticated.
...
async def test_update_post_success(client: AsyncClient):
    """
    Tests that a user can update their own post.
...
async def test_update_post_wrong_user(client: AsyncClient):
    """
    Tests that a user can't update another's post.
...
async def test_get_posts_with_pagination(client: AsyncClient):
    """
    Tests that pagination works when displaying posts.
    ...
    response = await client.get("/api/v1/posts?limit=2")
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 5
    ...
    response = await client.get("/api/v1/posts?skip=2&limit=2")
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 5
    assert len(data["posts"]) == 2
